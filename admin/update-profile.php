...
$sqlUser = "SELECT first_name, last_name, email, contact_no, profile_pic, password FROM admin WHERE admin_id = ?";
...
if (isset($_POST['remove_picture'])) {
    ...
    if (!empty($currentProfilePicture) && file_exists($targetFile)) {
        ...
        $sqlRemovePic = "UPDATE admin SET profile_pic = '' WHERE admin_id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['remove_picture'])) {
    ...
    if (!array_filter($errors)) {
        $sqlUpdateUser = "UPDATE admin SET first_name = ?, last_name = ?, email = ?, contact_no = ?, profile_pic = ?, password = ? WHERE admin_id = ?";
...
<head>
    ...
    <link rel="stylesheet" href="admin-css/popup-message.css">
